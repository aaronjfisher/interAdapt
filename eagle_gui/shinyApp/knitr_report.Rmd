...
```{r}
#BUGS? hrefs aren't working, ask leo? Knit citations? some of the Eqs work in RStudio, but not via knit()?
...
bibFile<-read.bibtex('EAGLEcite.bib')

#Leo's version of citep
...
This report was created using the EAGLE software for generating and analyzing trial designs with adaptive enrollment criteria. The EAGLE software can be accessed online at

http://spark.rstudio.com/mrosenblum/eagle_gui_demo

Additional documentation for the EAGLE software, including instructions on how to download EAGLE for personal offline use, can be found at

http://www.biostat.jhsph.edu/~afisher/AdaptiveShiny/About_Eagle.pdf

### Table of Contents:
* <a href=#Introduction > Introduction </a>
* <a href=#Summary of Input Data > Summary of Input Data </a>
* <a href=#Full List of Inputs > Full List of Inputs </a>
* <a href=#EAGLE Output > EAGLE Output </a>
  * <a href=#Decision Boundary Plots > Decision Boundary Plots </a>
  * <a href=#Performance Comparsion Plots > Performance Comparsion Plots </a>
...
* <a href=#Description of Input Variables > Description of Input Variables </a>
...
In this report, we consider the scenario where we have prior evidence that the treatment might work better in a one subpopulation than another. We use the term "adaptive design" to refer to a group sequential design that starts by enrolling from both subpopulations, and then decides whether or not to continue enrolling from each subpopulation based on interim analyses.  We use the term "standard design" to refer to a group sequential design where the enrollment criteria is fixed.

Below, we describe an adaptive design in more detail, and compare the performance of this design against that of standard designs. Performance is compared in terms of expected sample size, expected trial duration, and power, with family-wise type I error rate set to be constant (e.g. 0.05) for all three trials.
...
In the remained of this section we give an overview of the relevant concepts and notation needed to understand and use EAGLE. A more detailed discussion of the theoretical context, and of the parameter calculation procedure, can be found in `r mycitep(bibFile[["Rosenblum2013AdaptMISTIE"]])`.
...
Let $Z_{A,k}$ and $Z_{B,k}$ denote the analogous test statistics for the z-scores of the estimated treatment effect in subpopulations $A$ and $B$. The explicit form of $Z_{A,k}$ can be written as follows, where $A_{i,k}$ is an indicator that the $i ^{th}$ subject recruited in stage $k$ is a member of subpopulation $A$:
...
As described in `r mycitep(bibFile[["Rosenblum2013AdaptMISTIE"]])`, our decision rules in $AD$ consist of the following steps for each stage $k$:
...
This report was created using the "knitr" R package `r mycitep(bibFile[["knitr"]])`, with citations created using the "knitcitations" R package `r mycitep(bibFile[["knitcitations"]])`.
