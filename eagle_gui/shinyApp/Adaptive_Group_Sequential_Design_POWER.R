...
outcome_variance_subpop_2,return_cov_mat=FALSE)
{
	p1 <- p1_user_defined
	p2 <- 1-p1
...
global_cov_mat<<-cov_matrix_full
...
get_beta<-function(){
#beta[k] = prob of being above efficacy boundary for stage k, and between efficacy and futility boundary for stages 1:k.
#= prob of rejecting H0 at stage k.
	k_star<-last_stage_subpop_2_enrolled_adaptive_design
	
	beta<-list(AD_H0C=rep(NA,K),AD_H0S=rep(NA,K),FS=rep(NA,K),FC=rep(NA,K))
	#Boundaries
	ad_table<- adaptive_design_sample_sizes_and_boundaries_table()[[1]]
	ad_fck<- ad_table["H0C Futility Boundary", ]
	ad_eck<- ad_table["H0C Efficacy Boundary",]
	ad_fsk<- ad_table["H0S Futility Boundary",]
	ad_esk<- ad_table["H0S Efficacy Boundary",]

	for(k in 2:total_number_stages){
		lower<-c(ad_fsk[1:(k-1)],ad_esk[k])
		upper<-c(ad_esk[1:(k-1)],Inf)
		pmvnorm(lower=lower,upper=upper,mean=rep(true_diff,k),sigma=cov_matrix)
	}
