...
%We say that the $k^{th}$ stage has ended once a certain number of additional participants (denote $n_k$, defined below for each type of design) have been enrolled. %!!! OK def?
...
The decision boundaries for the design $SS$  are defined analogously as for the design $SC$, except using z-statistics $Z_{1,k}$. \pkg{interAdapt} makes the simplification that the number of participants $n_k$ enrolled in each stage $k$ of $SS$ is constant, denoted $n_{SS}$, input by the user.
...
%Stages after stage $k^*$ may require a longer or shorter amount time to complete, relative to the earlier stages, depending on whether the number of participants enrolled per stage from subpopulation 1 increases or decreases after stage $k^*$ (i.e., whether $n^{(2)}>\pi_1 n^{(1)}$ or $n^{(2)}<\pi_1 n^{(1)}$).
%!!! new
