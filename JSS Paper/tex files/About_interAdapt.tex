...
We introduce the \pkg{interAdapt} \proglang{R} package, a user friendly set of tools for exploring certain types of adaptive enrichment designs. The package contains a densely featured \pkg{shiny} application, ideal for users with little to no \proglang{R} programming experience, as well as a function allowing access to the core functionality of the \pkg{shiny} application directly in the \proglang{R} console. Several user inputs are available to allow the user to describe the context of his/her trial, and computations for generating output typically require less than 15 seconds on a standard commercial laptop. 
...
The \pkg{interAdapt} \proglang{R} package contains an interactive web application, as well as a command line function, both of which execute the same computations. The web application is built using the \pkg{shiny} package \citep{shiny2013manual}, with the user interface shown in a web browser, and the back-end calculations done in an \proglang{R} session.
...
Once \pkg{interAdapt} has been installed, the \pkg{shiny} application can be run without an internet connection by opening the \proglang{R} console and typing
...
The same calculations can be done directly in the \proglang{R} console, using the \code{compute_design_performance} function. Further details are provided in the \pkg{interAdapt} package documentation, and can be accessed by typing \code{help(package=interAdapt)} into the \proglang{R} console. 
The input arguments available when using this function have the same interpretation as the inputs available in the \pkg{shiny} application (see Section \ref{sub:inputs}). The function output contains the tables shown by the \pkg{shiny} application, which can be used to generate the plots shown by the application (see Section \ref{sub:outputs}).
