...
We introduce the \pkg{interAdapt} software tool, which includes a densely featured \pkg{shiny} application providing a user-friendly interface for exploring certain types of adaptive enrichment designs, and for comparing these to standard designs.
The \pkg{shiny} application can be either accessed online through a web browser, or run locally as an \proglang{R} package. Users can also access the core functionality of \pkg{interAdapt} via the \proglang{R} command line.
...
The same calculations can be done directly in the \proglang{R} console, using the \code{compute_design_performance} function. Further details are provided in the package documentation (i.e.\\
