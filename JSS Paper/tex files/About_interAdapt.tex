...
% **Hint**: If you want to use the JSS style for a non-JSS paper (or a modification of a JSS paper, e.g., in a vignette), you can set the option nojss in the \documentclass statement to suppress JSS-specific layout.
...
no programming experience. Several added features are available when
using the \pkg{shiny} application. For example, the application allows
users to immediately download the results of the performance
...
In Section \ref{sec:running-interAdapt}, we describe how to install \pkg{interAdapt} on a personal computer, run the \pkg{shiny} application locally, use the package functionality in the command line, and access the application online through a web browser. Section \ref{sec:UI} describes the inputs available when using \pkg{interAdapt}, and discusses the interpretation of the package's output. In Section \ref{sec:example}, we present an example demonstrating how an adaptive design is created and analyzed with \pkg{interAdapt}. 
...
% \item (2) Assess Futility in the Combined Population: Else, if $Z_{C,k}  \leq  l_{C,k}$, stop enrolling from subpopulation $2$ in all future stages. In this case when $Z_{1,k}>l_{1,k}$, the following additional steps must be done:
...
% 	\subitem  (b) If $Z_{1,k}  \leq  l_{1,k}$, we fail to reject either $H_{0C}$ or $H_{01}$, and stop all enrollment.
% 	\subitem  (c) Else, we continue to enroll from subpopulation $1$, and re-evaluate steps (2)-(3) at the end of the next stage. 
% \item  (3) If $l_{C,k} < Z_{C,k}  \leq  u_{C,k}$, continue enrolling from both subpopulations.
% \end{description}
...
The \pkg{interAdapt} \proglang{R} package contains an interactive web browser application, as well as a command line function which performs the same computations. The browser application is built on the R packages \pkg{shiny} \citep{shiny2013manual} and \pkg{RCurl} \citep{RCurlManual}, with the back-end calculations done in \proglang{R}.

To access the \pkg{shiny} application, \pkg{interAdapt} requires the user's default web browser to be set to either Firefox (\url{http://www.mozilla.org}) or Chrome (\url{http://www.google.com/chrome/}). Users can then run the application either by installing \proglang{R} and the \pkg{interAdapt} package locally on their computer, or by simply using Firefox or Google Chrome to view \pkg{interAdapt} online. Both options are free and quick to set up. However, because the online application will slow down noticeably if accessed by multiple users simultaneously, we encourage heavy users to install \pkg{interAdapt} locally.


\subsection[Running interAdapt over the web]{Running \pkg{interAdapt} over the web}
...
\subsection[Running interAdapt locally]{Running \pkg{interAdapt} locally}
\label{sub:running-locally}



To run \pkg{interAdapt} locally, one must first install the \proglang{R} programming language. \proglang{R} runs on both Windows \& MacOS, is available for download at (\url{http://www.r-project.org/}). After downloading and installing \proglang{R}, activating the \proglang{R} application will open an ``\proglang{R} console'' window where typed commands are executed by \proglang{R}. The \pkg{interAdapt} \proglang{R} package can be installed by typing \code{install.packages("interAdapt")} into the \proglang{R} console, while connected to the internet. Once \pkg{interAdapt} has been installed, the \pkg{shiny} application can be run without an internet connection by typing \code{library(interAdapt)} into the \proglang{R} console, and then typing \code{"yes"} (with quotation marks) when prompted on whether the application should be launched.

The same calculations that the application performs can also be done directly in the \proglang{R} console, with the \code{compute_design_performance} function. Further details are provided in the \pkg{interAdapt} package documentation, which can be accessed by typing \code{help(package = interAdapt)} into the \proglang{R} console. 
...
In this section, and in Section \ref{sec:example}, we will generally use the term \pkg{interAdapt} to refer to the \pkg{interAdapt} \pkg{shiny} application, although the inputs and outputs of the command line interface (see Section \ref{sub:running-locally}) have the same interpretation.
