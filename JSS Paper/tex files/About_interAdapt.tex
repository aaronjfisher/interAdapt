...
%change above from \documentclass[article]{jss} -> {article}; \Abstract{} -> \begin{abstract}...\end{abstract}; Code -> verbatim; \code{...} -> \verb!...!
%comment out Plaintitle, Shorttitle, Address, Keywords
%in JSS version abstract is in preamble. In basic version, it's in the document
...
The same calculations can be done directly in the \proglang{R} console, using the \code{compute_design_performance} function. Further details are provided in the \pkg{interAdapt} package documentation, which can be accessed by typing \code{help(package=interAdapt)} into the \proglang{R} console. 
The function's arguments are the same as the parameters available in the
\pkg{shiny} application (see Section \ref{sub:inputs}).
The function's value contains the output tables of the \pkg{shiny} application.
