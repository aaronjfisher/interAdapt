...
To access the \pkg{shiny} application, \pkg{interAdapt} requires the user's default web browser to be set to either Firefox (\url{http://www.mozilla.org}) or Chrome (\url{http://www.google.com/chrome/}). Users can then run the \pkg{shiny} application locally by installing \proglang{R} and the \pkg{interAdapt} package via CRAN. Loading the \pkg{interAdapt} package will automatically prompt the user on whether the \pkg{shiny} application should be loaded. 
