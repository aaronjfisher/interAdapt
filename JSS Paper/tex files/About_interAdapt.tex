...
\Abstract{
The \pkg{interAdapt} \proglang{R} package is designed to aid
statisticians and clinical investigators in planning randomized
trials.
It can be used to determine whether certain adaptive designs offer
tangible benefits compared to standard designs, in the context of
investigators' specific trial goals and constraints.
Specifically, \pkg{interAdapt} compares the performance of trial designs with adaptive
enrollment criteria to that of standard (non-adaptive) group sequential
designs.
Performance is evaluated in terms of power, expected trial duration,
and expected sample size.
Users can work interactively in the \proglang{R} console or with a user
friendly \pkg{shiny} application that requires no programming experience.
The \pkg{shiny} application contains several features for saving and
re-loading the results of intermediate steps of the analysis.
For example, the application allows users to save the results of the performance comparison as
a tabular csv-file or as a printable, html-based report.
}
...
The \pkg{interAdapt} \proglang{R} package contains an interactive web browser application, as well as a command line function which performs the same computations. The browser interface is built on the \pkg{shiny} \citep{shiny2013manual} and \pkg{RCurl} \citep{RCurlManual} packages, with the back-end calculations done in \proglang{R}.
